...
### Added
- 📐 **Angle Blood**, a second angular sector alongside the guide-wire shadow, for the stretch of a frame where blood obscures the image. It is the same tool with a dark red border (`intravascular.color_blood`): the 📐 **Angle Wire** button is now a dropdown offering **Angle Wire** and **Angle Blood**, and picking one recolours it, points ➕📐 **Add** at the same type (**Add Wire** / **Add Blood**) and starts one. `B` and `Ctrl+B` are its shortcuts, matching `3` / `Ctrl+3` for the wire. Blood is a contour type like any other: several per frame, exported into the mask as label 10, cleared by **Delete All On Frame**, and undone with `Ctrl+Z`. In the mask it is the bottom-most layer of all — every structure, the guide-wire shadow included, stays visible where it overlaps blood.
- 🗑️ **Delete All On Frame**, next to the contour-type controls: clears every contour, both measurements, the reference point and the wire angles on the frame on screen, along with the numbers derived from its lumen. The frame's phase and OCT label describe the frame rather than the drawing and are kept, as is every other frame. It records a single undo entry, so one `Ctrl+Z` brings the whole frame back.

### Changed
- **Angular sectors are placed by opening them, and stay editable.** The first click sets the boundary the sector opens from; two handles and an arc appear on a circle 5 mm out from the image centre (`intravascular.angle_handle_radius_mm`, exposed as **Angle Handle Radius** in Display Settings, clamped to stay inside the image), and the sector then opens as the pointer turns, its moving boundary dotted and its opening written in degrees beside the arc. The second click fixes it and the dotted boundary turns solid. Because the opening is tracked as the pointer turns rather than derived from the two clicks, a sector can now be opened **past 180°** — which two boundary points alone cannot express — and turning back through the first boundary opens it the other way instead. Either boundary can be dragged afterwards to re-aim it, with the other staying put; a drag past the other boundary collapses the sector rather than flipping it inside out. A sector is stored with a third point marking its interior, which is what records an opening wider than 180°; files written before that keep their original meaning (the smaller of the two arcs), so nothing needs migrating, and a sector is rewritten in the new shape the first time it is edited.
- The OCT right half is split into two rows: **Tagged Frame** / **Tagged Frames** now sit next to three new per-frame labels — **Guiding Catheter**, **Unanalyzable** and **Unlabeled** — and the quality rating moved to its own **Frame Quality** row below, both rows sharing one grid so the separators line up and each row spreads across the same width. The three new labels are toggle buttons like the quality ratings and belong to the same exclusive group as them, so a frame carries exactly one of the eight: picking any button clears whatever the frame had before. A frame no longer starts out rated *Very Good* — every frame starts **Unlabeled**.
- **Catheter Range**, past a divider at the end of the OCT frame labels: labels the current frame and every frame after it as **Guiding Catheter** in one click, for the tail of a pullback where the catheter fills the image. Each frame is relabelled exactly as a click would, replacing whatever it carried; frames before the current one are left alone. The vessel schematic veils the labelled stretch in translucent grey — nothing drawn under it is vessel. **Clear Catheter Range**, directly below it in the same action column, reverts the whole thing: every frame labelled as guiding catheter goes back to **Unlabeled**. It stays disabled while no frame carries the label.
- `FrameData` gains `guiding_catheter`, `unanalyzable` and `unlabeled`, and its `quality` now defaults to `''` (unrated) instead of *Very Good*. All four are written to the contour JSON like any other frame field, and exactly one of them is ever set — a file holding any other combination is normalised on load. Contour files from before 0.11.0 carried that *Very Good* default on every frame, reviewed or not, so they cannot say which frames were analyzed: on load the EEM stands in for it. A frame with one was analyzed and keeps whatever label the file assigned it; a frame without one comes back unrated and **Unlabeled**. A frame is never left with nothing at all: no rating and no flag means **Unlabeled**, whatever the file says.

- The per-frame report covers everything drawn on a frame, and is written as `<case>_report.csv` instead of the tab-separated `<case>_report.txt`. New columns: each plaque as an area **and** an angle (`calcium_area`/`calcium_angle`, and the same for lipid and macrophage), and `blood_angle` — every blood sector on the frame combined into one opening, overlap counted once. Plaque areas and angles are read off the rasterized mask rather than the contour polygons, so the report says exactly what the exported mask holds, and a plaque drawn as an open arc is measured over the wedge it actually fills. The guide-wire shadow is deliberately absent: it marks where the image cannot be read, not what is in the vessel. Both hand measurements now sit at the end of the per-frame columns, followed by the pullback's own parameters (modality, pullback speed, start frame, frame rate, resolution) as the last columns — repeated on every row instead of only the first, so a single row carries everything needed to read it.

### Fixed
- **Saving a report no longer looks finished while it still has minutes to run.** The progress bar covered the quick first pass over the frames only, so it filled up — and, a `QProgressDialog` hiding itself the moment it reaches its maximum, vanished — while the slow work and the file writing were still to come, with the success box arriving long afterwards. One dialog now spans every phase, names the one it is in (*Measuring contours* / *Measuring plaques and blood* / *Saving contour CSVs* / *Writing the report*), stays up until the last file is written, and cancels out of any of them. The slow phase itself is **~4x faster**: filled contours are rasterized by a scanline over the polygon's rows instead of `skimage.draw.polygon2mask`, which tests every pixel of the contour's bounding box against all ~500 of its smoothed vertices (~95 ms per contour, several per frame), and the per-pixel angle grid an open contour or an angular sector needs is built once per frame instead of once per contour. A 300-frame pullback with plaques on every frame went from 76 s to 19 s. The two fills agree to a pixel or two of boundary tangency out of tens of thousands, pinned against `polygon2mask` in the tests.
- **Tag Frames by Distance** now counts the spacing out from the frame on screen rather than from the first frame of the range, in both directions: on frame 375 of 380 with a step of 4 it tags 375, 379 and 371, 367, … back down. The frame being looked at is always one of the tagged ones, and a step in mm still spaces the frames evenly instead of drifting.
- **Saving a report crashed for every case**: `compute_all` called `_full_list` with two of its three arguments, so `Ctrl+R`, the results plot, the gating run and the breathing-sort viewer all raised `TypeError` before producing anything.
- A plaque contour drawn as a closed ring around the whole lumen — a circumferential calcification, say — was rasterized inside out: the disc it encloses was filled and then the lumen cut back out of it, so the mask showed calcium from the lumen out to the drawn ring instead of from the ring out to the EEM. A plaque never contains the lumen, so a contour that swallows it is read as the plaque's luminal boundary and the wall outside it is filled, which is what an open arc has always meant. Plaques with no EEM to fill outwards to, and closed contours drawn in the wall, are unchanged. `contours_to_mask` and `frame_region_areas` (the report and OCT schematic areas) now share one reading of a plaque contour instead of clipping one each.
...
### Fixed
- Contours were saved to a file the loader never reads back whenever the image file name contained a dot beyond its extension — the norm for OCT exports named after a DICOM UID (`1.2.840.…604688.dcm` saved as `1.2.840.…3_contours_*.json`). The save path stripped a second extension off the already extension-free stem that `read_contours` globs against, so the work looked saved and came back empty. Report, gated-image and video exports shared the same flaw and could collide between two pullbacks whose names differed only after the first dot.
- Edits other than contour-point drags now trigger a save of their own: every contour-changing operation (drawing, brush, copy from a neighbour, delete, `Ctrl+wheel` scaling, wire/reference/measurement placement, automatic segmentation, mask import, phase and OCT quality labels, breathing peak/valley edits, gating results) flags the frame data, and a save follows ~1.5 s after the edits stop. Pending changes are also written before the page is replaced by a newly opened file and before the app closes — both previously discarded anything newer than the last interval auto-save.
...
[0.10.0]: https://github.com/AI-in-Cardiovascular-Medicine/HolOrama/compare/v0.9.0...v0.10.0
