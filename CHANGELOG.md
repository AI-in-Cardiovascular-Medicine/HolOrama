...
- **Catheter Range**, next to the OCT frame labels: labels the current frame and every frame after it as **Guiding Catheter** in one click, for the tail of a pullback where the catheter fills the image. Each frame is relabelled exactly as a click would, replacing whatever it carried; frames before the current one are left alone.
