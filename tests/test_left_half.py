...
class TestLeftHalfFinal:
    ...
    def create_mock_main_window(self):
        ...
        mock_main_window.runtime_data.metadata = {'num_frames': 100}
        mock_main_window.runtime_data.gated_frames_dia = [10, 20, 30]
        mock_main_window.runtime_data.gated_frames_sys = [15, 25, 35]
    ...
    def test_left_half_creation(self):
        ...
        with patch('PyQt6.QtWidgets.QWidget') as MockWidget, patch('PyQt6.QtWidgets.QVBoxLayout') as MockVBox, patch(
            'PyQt6.QtWidgets.QHBoxLayout'
        ) as MockHBox, patch('PyQt6.QtWidgets.QGridLayout') as MockGrid, patch(
            'PyQt6.QtWidgets.QPushButton'
        ) as MockButton, patch(
            'PyQt6.QtWidgets.QLabel'
        ) as MockLabel, patch(
            'PyQt6.QtWidgets.QCheckBox'
        ) as MockCheckBox, patch(
            'PyQt6.QtCore.Qt'
        ), patch(
            'gui.left_half.left_half.Display'
        ) as MockDisplay, patch(
            'gui.left_half.left_half.Slider'
        ) as MockSlider, patch(
            'gui.left_half.left_half.Communicate'
        ):
    ...
    def test_left_half_methods_in_isolation(self):
        ...
        def change_value_side_effect(value):
            ...
            if value in left_half.main_window.runtime_data.gated_frames_dia:
                left_half.main_window.diastolic_frame_box.setChecked(True)
            else:
                left_half.main_window.diastolic_frame_box.setChecked(False)
                if value in left_half.main_window.runtime_data.gated_frames_sys:
...
class TestLeftHalfIntegration:
    ...
    def test_play_animation_flow(self):
        ...
        mock_main_window = MagicMock()
        mock_main_window.image_displayed = True
        mock_main_window.runtime_data.metadata = {'num_frames': 10}
        mock_main_window.display_slider = MagicMock()
