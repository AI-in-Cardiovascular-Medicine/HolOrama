...
class IVUSDisplay(QGraphicsView, MetricsMixin):
    ...
    def set_data(self, lumen, images):
        ...
        match ContourType:
            case ContourType.LUMEN:
                self._initialize_contour_data(num_frames=num_frames, key=ContourType.LUMEN.value)
            case ContourType.EEM:
                self._initialize_contour_data(num_frames=num_frames, key=ContourType.EEM.value)
            case ContourType.CALCIUM:
                self._initialize_contour_data(num_frames=num_frames, key=ContourType.CALCIUM.value)
            case ContourType.BRANCH:
                self._initialize_contour_data(num_frames=num_frames, key=ContourType.BRANCH.value)
            case _:
                pass

        self.finalized_splines = {ct.value: [None] * num_frames for ct in ContourType}

        self._build_spline_from_contour()
    ...
    def _initialize_contour_data(self, num_frames: int, key: str = None):
        """
    ...
    def _build_spline_from_contour(self):
        """
            ...
            try:
                if contour_data and contour_data[0][self.frame]:
                    ...
                    start_coords = (
                        ...
                        if self.main_window.data[f"{key}_start"][self.frame] is not None
                        else (xs[0], ys[0])
                    )
                    end_coords = self.main_window.data[f"{key}_end"][self.frame]
                    ...
                    spline = Spline(
                        geometry=geometry,
                        color=self.contour_configs[ct].color if ct in self.contour_configs else self.color_contour,
                        line_thickness=self.contour_thickness,
                        transparency=(
                            self.contour_configs[ct].alpha if ct in self.contour_configs else self.alpha_contour
                        ),
                    )
                    self.finalized_splines[key][self.frame] = spline
                else:
                    self.finalized_splines[key][self.frame] = None
            except Exception:
                self.finalized_splines[key][self.frame] = None
    ...
    def get_full_contour_list(self, contour_type: ContourType = None, unscaled: bool = False):
        ...
        """
        # if legacy list
        if not hasattr(self, "finalized_splines") or self.finalized_splines is None:
            return None
        # dict path (preferred): return list or None
        key = contour_type.value if contour_type else self.active_contour_type.value
        if key in self.finalized_splines:
            if unscaled:
                return [
                    spline.geometry.get_unscaled_contour() if spline else None for spline in self.finalized_splines[key]
                ]
            else:
    ...
    def get_current_spline(self):
        ...
        key = self.active_contour_type.value
        if key in self.finalized_splines:
            return self.finalized_splines[key][self.frame]
        return None
    ...
    def display_image(self, update_image=False, update_contours=False, update_phase=False):
        ...
        if self.main_window.hide_contours:
            ...
            if update_contours:
                lumen_contour = self.get_full_contour_list(ContourType.LUMEN, unscaled=True)
                eem_contour = self.get_full_contour_list(ContourType.EEM, unscaled=True)
                self._draw_contours()
                self._draw_measure()
    ...
    def start_contour(
        self, contour_type: ContourType = None, segmentation_tool: SegmentationTool = SegmentationTool.CLOSED_SPLINE
    ):
        """
        ...
        current_full_contour = self.get_full_contour_list(self.active_contour_type)
        self.main_window.tmp_contours[self.active_contour_type.value] = current_full_contour

        self.active_contour_type = contour_type if contour_type else self.active_contour_type
    ...
    def add_contour(self, click_pos, segmentation_tool: SegmentationTool = SegmentationTool.CLOSED_SPLINE):
        ...
        match segmentation_tool:
            case SegmentationTool.CLOSED_SPLINE:
    ...
    def stop_contour(self):
        ...
        if self.main_window.image_displayed:
            ...
            # store the finalized contour
            key = self.active_contour_type.value

            self.main_window.data[key][0][self.frame] = self.working_spline.geometry.full_contour[0].tolist()
            self.main_window.data[key][1][self.frame] = self.working_spline.geometry.full_contour[1].tolist()
            self.main_window.data[f"{key}_start"][self.frame] = (
                self.working_spline.geometry.start_coords
                if self.working_spline.geometry.start_coords
                else self.working_spline.geometry.full_contour[0][0].tolist()
            )
            self.main_window.data[f"{key}_end"][self.frame] = (
                self.working_spline.geometry.end_coords if self.working_spline.geometry.end_coords else None
            )

            self.finalized_splines[key][self.frame] = self.working_spline
            self._interrupt_drawing_mode()
    ...
    def _handle_item_interaction(self, scene_pos, view_pos):
        ...
        if point_item and point_item in current_spline.geometry.knot_points_items:
