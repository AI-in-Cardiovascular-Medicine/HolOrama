...
class CctaViewer3D(QWidget):
    ...
    # ── lasso erase (mechanics shared with Fusion's reclassify lasso via tools.lasso) ──
    ...
    def _lasso_add_point(self, sx: int, sy: int) -> None:
        if self._lasso.add_point(sx, sy):
            self._lasso_execute()
            return
        self._lasso.redraw()
        self._vtk_widget.GetRenderWindow().Render()
    ...
    def _erase_inside_lasso(self, label: int) -> None:
        assert self._mask is not None and self._voxel_spacing is not None
        dz, dy, dx = self._voxel_spacing
        ...
        screen = project_world_batch(self._ren, self._vtk_widget.GetRenderWindow().GetSize(), wx, wy, wz)
        inside = self._lasso.contains(screen)
        if not inside.any():
            return
